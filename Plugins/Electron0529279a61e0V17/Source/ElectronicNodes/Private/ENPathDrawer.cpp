...
void FENPathDrawer::DrawDefaultWire(const FVector2f& Start, const FVector2f& StartDirection, const FVector2f& End, const FVector2f& EndDirection)
{
	...
	DrawList.Add(TTuple<FVector2D, FVector2D, FVector2D, FVector2D>(Start, StartDirection * Tangent, End, EndDirection * Tangent));
...
float FENPathDrawer::GetRadiusOffset(const int32& AngleDeg, bool Perpendicular)
{
	...
	{
	case 45:
		RadiusOffset *= UE_HALF_SQRT_2;
		break;
	case 90:
		RadiusOffset *= 1.0f;
		break;
	case 135:
		RadiusOffset *= (1.0f - (UE_HALF_SQRT_2));
		break;
	case 180:
		RadiusOffset *= 2.0f;
		break;
...
float FENPathDrawer::GetIntersectionOffset(const int32& AngleDeg, bool Diagonal)
{
	...
	case 45:
		if (Diagonal)
		{
			IntersectionOffset *= (1.0f - UE_HALF_SQRT_2) * UE_SQRT_2;
		}
		else
		{
			IntersectionOffset *= UE_SQRT_2 - 1.0f;
		}
		break;
	case 90:
...
void FENPathDrawer::DrawLine(const FVector2f& Start, const FVector2f& End, int64 MaxSplit)
{
	if (FMath::IsNearlyZero((End - Start).SizeSquared(), KINDA_SMALL_NUMBER))
	{
		return;
	...
	{		
		for (ENCrossingConnection CrossingConnection : ConnectionDrawingPolicy->CrossingConnections)
		{
			// Maybe check if collinear first?
			
			FVector2f OutIntersectionPoint;
			const bool bIntersection = ENIntersectionHelpers::SegmentIntersection2D(
				CrossingConnection.Start, CrossingConnection.End, Start, End, OutIntersectionPoint);
			...
			const float DistanceToStart =  (OutIntersectionPoint - Start).Size();
			const float DistanceToEnd = (OutIntersectionPoint - End).Size();
			...
			if (DistanceToStart < MinOffset || DistanceToEnd < MinOffset)
			{
				continue;
			}
			
			const FVector2f BeforeIntersectionPoint = OutIntersectionPoint - ((OutIntersectionPoint - Start).GetSafeNormal() * CrossingSize);
			const FVector2f AfterIntersectionPoint = OutIntersectionPoint - ((OutIntersectionPoint - End).GetSafeNormal() * CrossingSize);
			
			DrawLine(Start, BeforeIntersectionPoint, MaxSplit - 1);
			DrawLine(AfterIntersectionPoint, End, MaxSplit - 1);
			
			if (ElectronicNodesSettings.CrossingStyle == ECrossingStyle::Arc)
			{
				const FVector2f NormalVector = (End-Start).GetSafeNormal().GetRotated(-90.0f) * CrossingSize / (10.0f * ZoomFactor);
				DrawSpline(BeforeIntersectionPoint, NormalVector,AfterIntersectionPoint,-NormalVector);
			}
	...
	ConnectionDrawingPolicy->CrossingConnections.Add(ENCrossingConnection(Start, End));
	DrawList.Add(TTuple<FVector2D, FVector2D, FVector2D, FVector2D>(Start, FVector2f::ZeroVector, End, FVector2f::ZeroVector));
}
...
void FENPathDrawer::DoDraw(const FLinearColor &WireColor)
{	
	for (auto &DrawElem : DrawList)
	{
		ConnectionDrawingPolicy->MakeDrawSpline(*DrawElementsList, LayerId,
			DrawElem.Get<0>(), DrawElem.Get<1>(), DrawElem.Get<2>(), DrawElem.Get<3>(), 
