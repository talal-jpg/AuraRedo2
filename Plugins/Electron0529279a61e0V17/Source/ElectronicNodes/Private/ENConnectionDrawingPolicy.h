...
struct ENCrossingConnection
{
	...
	ENCrossingConnection(FVector2f Start, FVector2f End)
	{
		this->Start = Start;
		this->End = End;
	}
...
struct FENConnectionDrawingPolicyFactory : public FGraphPanelPinConnectionFactory
{
	...
	virtual class FConnectionDrawingPolicy* CreateConnectionPolicy(const class UEdGraphSchema* Schema,
...
{
public:
	FENConnectionDrawingPolicy(int32 InBackLayerID, int32 InFrontLayerID, float ZoomFactor,
	                           const FSlateRect& InClippingRect, FSlateWindowElementList& InDrawElements,
	                           UEdGraph* InGraphObj, bool IsTree = false)
		: FKismetConnectionDrawingPolicy(InBackLayerID, InFrontLayerID, ZoomFactor, InClippingRect, InDrawElements,
		                                 InGraphObj), IsTree(IsTree)
	{
	}

	ELECTRONICNODES_API virtual void DrawConnection(int32 LayerId, const FVector2f& Start, const FVector2f& End,
	                                                const FConnectionParams& Params) override;

	void ENComputeClosestPoint(const FVector2f& Start, const FVector2f& End);
	void ENComputeClosestPointDefault(const FVector2f& Start, const FVector2f& StartTangent, const FVector2f& End,
	                                  const FVector2f& EndTangent);
	void ENDrawBubbles(const FVector2f& Start, const FVector2f& StartTangent, const FVector2f& End,
	                   const FVector2f& EndTangent);
	...
	static bool IsRibbonEqual(const float RibbonAMin, const float RibbonAMax, const float RibbonBMin,
	                          const float RibbonBMax);

	ELECTRONICNODES_API virtual void MakeDrawSpline(FSlateWindowElementList& ElementList, uint32 InLayer,
...
private:
	...
	void ENProcessRibbon(FENPathDrawer* PathDrawer, int32 LayerId, FVector2f& Start, FVector2f& StartDirection,
	                     FVector2f& End, FVector2f& EndDirection, const FConnectionParams& Params);
	bool ENIsRightPriority(const FConnectionParams& Params);
	int32 ENGetZoomLevel();
	int8 ENGetPinMembersCount(const UEdGraphPin* Pin);
	void ENDrawMainWire(FENPathDrawer* PathDrawer, EWireStyle WireStyle, FVector2f& Start, FVector2f& StartDirection,
	                    FVector2f& End, FVector2f& EndDirection, const FConnectionParams& Params);
