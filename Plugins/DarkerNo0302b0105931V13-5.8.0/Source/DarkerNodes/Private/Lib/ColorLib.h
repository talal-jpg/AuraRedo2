...
{
public:
	static float PivotRGBtoXYZ(const float N)
	{
		return (N > 0.04045f ? FMath::Pow((N + 0.055f) / 1.055f, 2.4f) : N / 12.92f) * 100.0f;
	}

	static FVector RGBtoXYZ(const FColor &Color)
	{
	...
	static FColor XYZtoRGB(const FVector &Color)
	{
		const FMatrix Matrix(
	...
	static float MathXYZtoLAB(const float T)
	{
	...
	static FVector XYZtoLAB(const FVector &Color)
	{
	...
	static float MathLABtoXYZ(const float T)
	{
		const float D = 6.0f / 29.0f;
		if (T > D)
		{
	...
	static FVector LABtoXYZ(const FVector &Color)
	{
		const float Xn = 95.0489f;
		const float Yn = 100.0f;
		const float Zn = 108.884f;
	...
	static FVector RGBtoHSL(const FColor &Color)
	{
	...
	static FColor HSLtoRGB(const FVector &Color)
	{
		...
		FVector OutColor = FVector(0,0,0);
